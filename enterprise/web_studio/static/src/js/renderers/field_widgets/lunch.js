odoo.define('web_studio.lunch', function (require) {
"use strict";
    ...
    render: function() {
        ...
        this.trigger_up('perform_model_rpc', {
            ...
            on_success: function (orders) {
                if (_.isEmpty(orders)) {
                    self.$el.html(QWeb.render("lunch_order_widget_no_previous_order"));
                } else {
                    _.each(orders, function(order) {
                        self.lunch_data[order.id] = order;
                    });
                    var categories = _.groupBy(orders, function(o){ return o.supplier[1]; });
                    return self.$el.html(QWeb.render("Newlunch_order_widget_previous_orders_list", {'widget': self, 'categories': categories}));
                }
            }
        });
    },
