...
class HmrcVatObligation(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', required=True,
        default=lambda self: self.env['res.company']._company_default_get('l10n_uk.vat.obligation'))
    ...
    def import_vat_obligations(self):
        ...
        vat = self.env.user.company_id.vat and self.env.user.company_id.vat[2:] or ''
        if not vat:
    ...
    def _fetch_values_from_report(self, lines):
        translation_table = {
            ...
            'vatReclaimedCurrPeriod': 'financial_report_line_uk_0103',
            'totalValueSalesExVAT': 'financial_report_line_uk_02',
            'totalValuePurchasesExVAT': 'financial_report_line_uk_03',
            'totalValueGoodsSuppliedExVAT': 'financial_report_line_uk_0201',
        ...
        for line in lines:
            if reverse_table.get(line['id']):
                ...
                value = line['columns'][0][2:].replace(',', '')
                value = 0.0 if value == '' else float(value)
                if reverse_table[line['id']] == 'netVatDue':
                    values[reverse_table[line['id']]] = abs(round(value, 2))
                    ...
                    values[reverse_table[line['id']]] = round(value)
                else:
                    values[reverse_table[line['id']]] = round(value, 2)
    ...
    def action_submit_vat_return(self):
        ...
        context_rec = self.env['account.financial.html.report.context'].create({'report_id': report.id})
        context_rec.write({'date_from': self.date_start, 'date_to': self.date_end,})
        report_values = report.get_lines(context_rec.id)
        values = self._fetch_values_from_report(report_values)
        vat = self.env.user.company_id.vat[2:] # In v10, the uk vat number needs to start with GB
        res = self.env['hmrc.service']._login()
        if res: # If you can not login, return url for re-login
            return res
