...
class AssignLocationToAccount(models.Model):
    ...
    name = fields.Char(string='Project', index=True, required=True, track_visibility='onchange')
...
class AccountVoucher(models.Model):
    ...
    beneficiary_account_id = fields.Many2one('res.partner.bank', string='Beneficiary Account Number')
    bank_id = fields.Many2one('res.bank', string='Bank Name')

    @api.model
    def create(self, vals):
        if vals.get('voucher_type', False) and vals['voucher_type'] == 'purchase':
            vals['number'] = self.env['ir.sequence'].next_by_code('account.voucher.purchase')
        return super(AccountVoucher, self).create(vals)

    @api.onchange('partner_id')
    def onchange_partner_bank_account(self):
        self.beneficiary_account_id = False
        self.bank_id = False
        if self.partner_id.bank_ids:
            for bank_id in self.partner_id.bank_ids:
                self.beneficiary_account_id = bank_id.id or False
                self.bank_id = bank_id.bank_id and bank_id.bank_id.id or False
                break
            
    @api.onchange('beneficiary_account_id')
    def onchange_beneficiary_account_id(self):
        self.bank_id = False
        if self.beneficiary_account_id.bank_id:
            self.bank_id = self.beneficiary_account_id.bank_id and self.beneficiary_account_id.bank_id.id or False
...
class AccountVoucherLine(models.Model):
    ...
    def product_id_change(self, product_id, partner_id=False, price_unit=False, company_id=None, currency_id=None,type=None):
        vals = super(AccountVoucherLine, self).product_id_change(product_id, partner_id=partner_id, price_unit=price_unit, company_id=company_id, currency_id=currency_id,type=type)
        if vals.get('value', False) and vals['value'].get('name'):
            vals['value']['name'] = ''
        return vals


class AccountVoucherReceiptGenerate(models.TransientModel):
    _name = "account.voucher.receipt.generate"

    voucher_ids = fields.Many2many('account.voucher', string="Voucher Receipts")

    def print_receipt(self):
        return self.env.ref('posh_multilocation.report_account_voucher_receipt').report_action(self.voucher_ids.ids)
